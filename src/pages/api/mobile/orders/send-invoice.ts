...
export const POST: APIRoute = async ({ request }) => {
    try {
        ...
        if (!order.invoice_number) {
            return new Response(JSON.stringify({ error: "Factura no generada aún" }), { status: 400 });
        }
        ...
        await sendOfficialInvoiceEmail(order, items);

        return new Response(JSON.stringify({ success: true }), { status: 200 });
    } catch (error: any) {
        console.error("Error en send-invoice:", error);
